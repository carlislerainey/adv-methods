...
* **Do** use a computer to simulate from some of these random variables. Choose at least one discrete and one continuous random variable and print, say 10, random draws to get a feel for what the numbers look like. In R, you'll use `rnorm()`, `rgamma()`, etc. Then tabulate, say 1,000, draws to get a sense for what the distribution looks like (i.e., `table(x)`). Then plot the distribution. Use a bar plot for discrete (i.e., `barplot(table(x))`), histogram for continuous (i.e., `hist(x)`). 
* **Do** graph some of these distributions in R. You should probably graph the discrete random variables as bar plots (i.e., `barplot(dbinom(0:10, 10, 0.5))`) and the continuous random variables as line plots (i.e., `curve(dnorm(x, 1, 1.5), xlim = c(-3, 3))`). Put several distributions on the same plot to get a sense of how the function changes as the parameters or family changes.
